...
public class RegistryController {
    ...
    public RegistryController(@Autowired GameServers gameServers, @Autowired TokenController tokenController) {
    ...
    public ResponseEntity registerGameService(@PathVariable String gamename, @RequestBody GameServerParameters gameServiceForm, Principal principal, @RequestParam("access_token") String token) {

        try {
            if (!tokenController.currentUserRole(token).toString().contains("SERVICE")) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Service token required.");
            }
            gameServiceForm.validate();
            gameServers.registerGameServer(gamename, gameServiceForm, tokenController.nameFromToken(token));
    ...
    public ResponseEntity unregisterGameService(@PathVariable String gameServiceName, @RequestParam("access_token") String token) {
        try {
            if (!tokenController.currentUserRole(token).toString().contains("SERVICE") && !tokenController.currentUserRole(token).toString().contains("ADMIN")) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Service or admin token required.");
            }
