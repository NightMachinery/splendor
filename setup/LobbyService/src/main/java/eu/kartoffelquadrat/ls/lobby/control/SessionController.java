...
import eu.kartoffelquadrat.ls.accountmanager.controller.RoomTokenService;
...
public class SessionController {
    ...
    public ResponseEntity createSession(@RequestBody CreateGameForm createGameForm, Principal principal, @RequestParam("access_token") String token, @RequestParam(required = false) String location) {
        ...
        try {
            ...
            String tokenName = tokenController.nameFromToken(token);
            if (tokenName.isEmpty() || !createGameForm.getCreator().equals(tokenName))
        ...
        String creator = tokenController.nameFromToken(token);
        Session session;
        ...
        setSessionDisplayName(session, creator);
    ...
    private void setSessionDisplayName(Session session, String playerName) {
        Optional<Player> player = playerRepository.findById(playerName);
        if (player.isPresent()) {
            String displayName = player.get().getDisplayName();
            session.setDisplayName(playerName, displayName == null || displayName.trim().isEmpty() ? playerName : displayName);
        } else {
            session.setDisplayName(playerName, playerName);
        }
    }


    @PostMapping("/api/sessions/{sessionid}/players/{player}/migrate-token")
    public ResponseEntity createMigrateToken(@PathVariable long sessionid, @PathVariable String player, @RequestParam("access_token") String token) {
        ...
        session.updateTempMods();
        String actor = tokenController.nameFromToken(token);
        if (!actor.equals(player) && !session.isMod(actor)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Only mods can copy another player's migrate link.");
        }
        if (!session.getPlayers().contains(player)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Player is not in this session.");
        }
        return ResponseEntity.status(HttpStatus.OK).body(roomTokenService.create(String.valueOf(sessionid), player));
    }

    @GetMapping("/api/sessions/{sessionid}/migrate/{migrateToken}")
    public ResponseEntity resolveMigrateToken(@PathVariable long sessionid, @PathVariable String migrateToken) {
        if (!roomTokenService.isForRoom(migrateToken, String.valueOf(sessionid))) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Invalid migrate token for room.");
        }
        String username = roomTokenService.username(migrateToken);
        Optional<Player> player = playerRepository.findById(username);
        if (!player.isPresent()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Migrated user no longer exists.");
        }
        return ResponseEntity.status(HttpStatus.OK).body(player.get());
    }

    @PostMapping("/api/sessions/{sessionid}/players/{player}/observer")
    public ResponseEntity setObserver(@PathVariable long sessionid, @PathVariable String player, @RequestParam("access_token") String token, @RequestParam("observer") boolean observer) {
        if (!sessions.isExistent(sessionid)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Session does not exist.");
        }
        Session session = sessions.getSessionById(sessionid);
        ...
        String actor = tokenController.nameFromToken(token);
        if (!actor.equals(player) && !session.isMod(actor)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Only mods can change another player observer status.");
        }
        session.setObserver(player, observer);
        sessionBroadcastManager.touch();
        sessionSpecificBroadcastManagers.get(sessionid).touch();
        return ResponseEntity.status(HttpStatus.OK).body(null);
    }

    @PostMapping("/api/sessions/{sessionid}/players/{player}/mod")
    public ResponseEntity setMod(@PathVariable long sessionid, @PathVariable String player, @RequestParam("access_token") String token, @RequestParam("mod") boolean mod) {
        if (!sessions.isExistent(sessionid)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Session does not exist.");
        }
        Session session = sessions.getSessionById(sessionid);
        session.updateTempMods();
        String actor = tokenController.nameFromToken(token);
        if (!session.isMod(actor)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Only mods can change mods.");
        }
        if (mod) {
            session.promoteMod(player);
        } else if (session.getCreator().equals(actor)) {
            session.demoteMod(player);
        } else {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Only the creator can demote mods in this build.");
        }
        sessionBroadcastManager.touch();
        sessionSpecificBroadcastManagers.get(sessionid).touch();
        return ResponseEntity.status(HttpStatus.OK).body(null);
    ...
    public ResponseEntity removeSession(@PathVariable long sessionid, Principal principal, @RequestParam("access_token") String token) {
        ...
        Session session = sessions.getSessionById(sessionid);
        if (!session.isLaunched() && !session.getCreator().equals(tokenController.nameFromToken(token)))
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Unlaunched sessions can only be deleted by creator.");
        String callerRole = tokenController.currentUserRole(token).toString();
        ...
        if( session.isLaunched() && callerRole.contains("SERVICE"))
            if(!tokenController.nameFromToken(token).equals(gameServers.getRegistringServiceAccountForGame(session.getGameName())))
    ...
    public DeferredResult<ResponseEntity<String>> getAllGamesUpdate(@RequestParam(required = false) String hash) {
        ...
        for (Session session : sessions.allSessions()) {
            session.updateTempMods();
        }
    ...
    public ResponseEntity joinSession(@PathVariable long sessionid, @PathVariable String player, Principal principal, @RequestParam("access_token") String token, @RequestParam(required = false) String location) {

        // Reject if registration occurs on behalf of someone else
        if (!player.equals(tokenController.nameFromToken(token)))
        ...
        setSessionDisplayName(session, player);
    ...
    public ResponseEntity leaveSession(@PathVariable long sessionid, @PathVariable String player, Principal principal, @RequestParam("access_token") String token) {
    ...
    public ResponseEntity launchSession(@PathVariable long sessionid, Principal principal, @RequestParam("access_token") String token) throws RegistryException {
        // Reject if session id does not exist
        if (!sessions.isExistent(sessionid))
        ...
        Session session = sessions.getSessionById(sessionid);
        if (!tokenController.nameFromToken(token).equals(session.getCreator()))
        ...
        if (session.getActivePlayers().size() < session.getGameParameters().getMinSessionPlayers())
    ...
    private void notifyGameLaunch(long sessionid, String gamename, Session session) throws RegistryException {
        ...
        for (String player : session.getActivePlayers()) {
